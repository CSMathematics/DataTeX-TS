...
export const PstricksWizard: React.FC<PstricksWizardProps> = ({
  onInsert,
  onChange,
}) => {
  ...
  useEffect(() => {
    ...
    let code = `\\usepackage${
      pkgOpts.length > 0 ? `[${pkgOpts.join(",")}]` : ""
    }{pstricks}\n`;

    let pssetOpts = [];
    if (unit !== 1 || unitType !== "cm")
      pssetOpts.push(`unit=${unit}${unitType}`);
    if (linewidth !== 0.8) pssetOpts.push(`linewidth=${linewidth}pt`);
    if (linecolor !== "black")
      pssetOpts.push(`linecolor=${linecolor.replace("#", "")}`);
    if (fillstyle !== "none") pssetOpts.push(`fillstyle=${fillstyle}`);
    if (fillcolor && fillstyle !== "none")
      pssetOpts.push(`fillcolor=${fillcolor.replace("#", "")}`);
    ...
    >
      ...
      >
        <Tabs.List>
          <Tabs.Tab
            value="config"
            leftSection={<FontAwesomeIcon icon={faCogs} />}
          >
            Configuration
          </Tabs.Tab>
          <Tabs.Tab
      ...
      <ScrollArea style={{ flex: 1 }} p="md">
        {activeTab === "config" && (
          <Stack gap="lg">
            <Box>
              <Divider
                label="Package Loading Options"
                labelPosition="left"
                mb="sm"
              />
              <Stack gap="xs">
                <Checkbox
                  label="No XColor (noxcolor)"
                  checked={noXcolor}
                  onChange={(e) => setNoXcolor(e.currentTarget.checked)}
                />
                <Checkbox
                  label="Plain TeX (plain)"
                  checked={plain}
              ...
              <Stack gap="md">
                ...
                <SimpleGrid cols={2}>
                  <Select
                    label="Fill Style"
                    value={fillstyle}
                    onChange={(v) => v && setFillstyle(v)}
                    data={["none", "solid", "vlines", "hlines", "crosshatch"]}
                  />
                  <TextInput
                    label="Fill Color"
                    value={fillcolor}
                    onChange={(e) => setFillcolor(e.currentTarget.value)}
                    disabled={fillstyle === "none"}
                    placeholder="blue!20"
                  />
                </SimpleGrid>
        ...
        {activeTab === "snippets" && (
          <Stack gap="md">
            <Text size="sm" c="dimmed">
              Click to insert common PSTricks commands into your document.
            </Text>
            ...
            <SimpleGrid cols={2}>
              <Button
                variant="default"
                size="xs"
                onClick={() => insertSnippet("\\psline(0,0)(3,2)")}
              >
                Line
              </Button>
              <Button
                variant="default"
                size="xs"
                onClick={() => insertSnippet("\\psframe(0,0)(4,2)")}
              >
                Frame (Rect)
              </Button>
              <Button
                variant="default"
                size="xs"
                onClick={() => insertSnippet("\\pscircle(2,2){1.5}")}
              >
                Circle
              </Button>
              <Button
                variant="default"
                size="xs"
                onClick={() => insertSnippet("\\psellipse(2,1)(1.5,0.8)")}
              >
                Ellipse
              </Button>
            </SimpleGrid>
            ...
            <SimpleGrid cols={2}>
              <Button
                variant="default"
                size="xs"
                onClick={() => insertSnippet("\\pscurve(0,0)(1,2)(2,1)(4,2)")}
              >
                Curve
              </Button>
              <Button
                variant="default"
                size="xs"
                onClick={() => insertSnippet("\\psarc(0,0){2}{0}{90}")}
              >
                Arc
              </Button>
              <Button
                variant="default"
                size="xs"
                onClick={() => insertSnippet("\\psbezier(0,0)(1,2)(3,2)(4,0)")}
              >
                Bezier
              </Button>
              <Button
                variant="default"
                size="xs"
                onClick={() => insertSnippet("\\pswedge(0,0){2}{0}{60}")}
              >
                Wedge
              </Button>
            </SimpleGrid>
            ...
            <SimpleGrid cols={2}>
              <Button
                ...
                onClick={() =>
                  insertSnippet(
                    "\\psgrid[subgriddiv=1,griddots=10,gridlabels=7pt](0,0)(4,4)"
                  )
                }
              >
                Grid
              ...
              <Button
                variant="default"
                size="xs"
                onClick={() => insertSnippet("\\psaxes{->}(0,0)(5,5)")}
              >
                Axes
              </Button>
            </SimpleGrid>
            ...
            <Button
              variant="default"
              size="xs"
              onClick={() => insertSnippet("\\rput(2,2){Label}")}
            >
              rput (Rotate/Put)
            </Button>
            <Button
              variant="default"
              size="xs"
              onClick={() => insertSnippet("\\uput[45](2,2){Label}")}
            >
              uput (Unit Put)
            </Button>
          </Stack>

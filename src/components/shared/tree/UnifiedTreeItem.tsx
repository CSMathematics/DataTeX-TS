...
import { Box, UnstyledButton, Group, Text, Collapse, Menu, TextInput, Checkbox } from '@mantine/core';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import {
    faChevronRight, faChevronDown, faFolder, faFolderOpen,
    faFileCode, faFilePdf, faBookOpen, faCog, faImage, faFile,
    faBoxOpen, faFileCirclePlus, faFolderPlus, faPen, faCopy,
    faTrash, faMinusSquare
...
export const getFileIcon = (name: string, type: 'file' | 'folder', expanded: boolean = false) => {
    ...
    switch (ext) {
        ...
        case 'svg':
        case 'webp':
            return <FontAwesomeIcon icon={faImage} style={{ width: 14, height: 14, color: "#40c057" }} />;
...
const InlineInput = React.memo<InlineInputProps>(({
    type,
    defaultValue = '',
    placeholder,
    onCommit,
    ...
    return (
        <Group gap={6} wrap="nowrap" px={8} py={4} style={{ paddingLeft: 20 }}>
            <FontAwesomeIcon
                ...
                style={{ width: 16, height: 16, color: type === 'folder' ? "#fab005" : "#4dabf7" }}
            />
            <TextInput
                ref={inputRef}
                size="xs"
                ...
                onBlur={onCancel}
                placeholder={placeholder || (type === 'file' ? 'filename.tex' : 'folder_name')}
                styles={{
                    input: {
                        height: 18,
                        minHeight: 18,
...
export const UnifiedTreeItem = React.memo<UnifiedTreeItemProps>(({
    ...
    const { attributes, listeners, setNodeRef: setDragNodeRef, isDragging } = useDraggable(dragConfig);
    ...
    const handleRenameCommit = useCallback((newName: string) => {
        setIsRenaming(false);
    ...
    const bgColor = useMemo(() => {
        if (isRoot) return 'var(--app-header-bg)';
        if (isSelected) return 'var(--mantine-primary-color-light)';
        return 'transparent';
    }, [isRoot, isSelected]);
        ...
        >
            <Group gap={isRoot ? 8 : 6} wrap="nowrap" style={{ flex: 1, overflow: 'hidden' }}>
                ...
                {node.type === 'folder' && (
                    <Box style={{
                        transform: expanded 
                            ? (isRoot ? 'rotate(0deg)' : 'rotate(90deg)') 
                            : (isRoot ? 'rotate(-90deg)' : 'none'),
                ...
                {isRoot && config.rootIcon ? (
                    ...
                    <FontAwesomeIcon
                        icon={faBoxOpen}
                        style={{ width: 14, height: 14, color: "var(--mantine-primary-color-filled)" }}
                    />
                ) : (
        ...
        <Menu shadow="md" width={200} opened={menuOpened} onChange={setMenuOpened} trigger="click">
            ...
            <Menu.Dropdown onContextMenu={(e) => e.preventDefault()}>
                ...
                {node.type === 'folder' && (
                    <>
                        <Menu.Item
                            leftSection={<FontAwesomeIcon icon={faFileCirclePlus} style={{ width: 14, height: 14 }} />}
                            onClick={() => callbacks.onCreate?.('file', node)}
                        >
                            New File
                        </Menu.Item>
                        <Menu.Item
                            leftSection={<FontAwesomeIcon icon={faFolderPlus} style={{ width: 14, height: 14 }} />}
                ...
                {isRoot && callbacks.onRemoveFolder && (
                    <>
                        <Menu.Item
                            color="orange"
                            leftSection={<FontAwesomeIcon icon={faMinusSquare} style={{ width: 14, height: 14 }} />}
                ...
                {!isRoot && (
                    <>
                        {config.enableRename && (
                            <Menu.Item
                                leftSection={<FontAwesomeIcon icon={faPen} style={{ width: 14, height: 14 }} />}
                        ...
                        <Menu.Item
                            leftSection={<FontAwesomeIcon icon={faCopy} style={{ width: 14, height: 14 }} />}
                        ...
                        <Menu.Divider />
                        <Menu.Item
                            leftSection={<FontAwesomeIcon icon={faTrash} style={{ width: 14, height: 14 }} />}
                            color="red"
    ...
    return (
        <Box ref={config.enableDragDrop ? setDropNodeRef : undefined} style={dropStyle}>
            {wrappedContent}
            ...
            <Collapse in={expanded}>
                ...
                {isCreatingHere && creatingState && onCommitCreation && onCancelCreation && (
                    <Box pl={(level + 1) * 12 + 8}>
                        <InlineInput
                            type={creatingState.type}
                            onCommit={(name) => onCommitCreation(name, creatingState.type, node.path)}
